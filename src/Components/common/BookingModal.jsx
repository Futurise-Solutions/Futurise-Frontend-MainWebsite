import { Box, Flex, Text, Icon, HStack, Modal, ModalOverlay, ModalContent, ModalHeader, ModalBody, ModalCloseButton, Spinner } from "@chakra-ui/react";
...
const BookingModal = () => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="5xl" isCentered scrollBehavior="inside">
      ...
      <ModalContent bg="bg.card" border="1px solid" borderColor="border.subtle" borderRadius="20px" overflow="hidden" mx={4}>
        <ModalHeader borderBottom="1px solid" borderColor="border.subtle" pb={4}>
          <HStack spacing={3}>
            <Flex w="36px" h="36px" borderRadius="10px" align="center" justify="center" bgGradient="linear(135deg, rgba(123,108,255,0.3), rgba(181,123,255,0.3))" border="1px solid" borderColor="rgba(180,160,255,0.3)">
              <Icon as={FiCalendar} color="accent.solid" boxSize={4} />
            </Flex>
            <Box>
              <Text fontSize="md" fontWeight={700}>Book a free strategy call</Text>
              <Text fontSize="xs" color="text.faint" fontWeight={400}>60 minutes · Futurise Solutions</Text>
            </Box>
          </HStack>
        </ModalHeader>
        ...
        <ModalBody p={0}>
          ...
          <Box position="relative" h={{ base: "480px", md: "580px" }}>
            {!iframeLoaded && (
              <Flex position="absolute" inset={0} align="center" justify="center" direction="column" gap={3} bg="bg.card" zIndex={1}>
                <Spinner color="accent.solid" size="xl" thickness="3px" speed="0.8s" />
                <Text fontSize="sm" color="text.faint">Loading calendar…</Text>
              </Flex>
            )}
            <Box
              as="iframe"
